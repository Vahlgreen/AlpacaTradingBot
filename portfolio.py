...
class Portfolio:
    def __init__(self, start_date: str, end_date: str,strategies: list, funds: float, transaction_fee: float,
                 risk_tolerance: float = 0.5, live: bool = False, client: TradingClient = None):
        self.funds = funds
        self.live = live
        self.client = client
        self.asset_value = 0
        ...
        self.validation_log = []
        self.strategies = [getattr(strategy, name)(funds=funds, name=name, strats=strategies, live=live, client=client) for name in strategies] # instantiate strategy objects
        ...
        if live and client is None:
            raise TypeError("If portfolio is deployed in a live environment, client can't be none.")
